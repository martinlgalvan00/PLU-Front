...
describe('APIs de dominio unificadas', () => {
  it('crea la compra publica por el repositorio Supabase y devuelve el token de orden', async () => {
    const createOrder = vi.fn().mockResolvedValue({
      order: { id: 'ord-1', amount: 20000 },
      tickets: [],
      orderAccessToken: 'opaque-token',
    })
    const target = listen(
      createApp({
        ...
        attendees: [
          {
            fullName: 'Ana Perez',
            dni: '30111222',
            ticketTypeId: crypto.randomUUID(),
            addonIds: [],
          },
        ],
      }),
  ...
  it('no permite consultar ni procesar una orden de pago solo por conocer su UUID', async () => {
    const paymentRepository = {
      getOrder: vi
        .fn()
        .mockResolvedValue({ id: '8cb43d94-b330-4e69-a2d0-76a56916ebf5', kind: 'ticket' }),
      assertTicketOrderAccess: vi
        .fn()
        .mockRejectedValue(new HttpError(401, 'Falta el token de acceso de la orden.')),
    }
    const target = listen(createApp({ paymentRepository }))
    const response = await fetch(
      `${target.url}/api/payments/orders/8cb43d94-b330-4e69-a2d0-76a56916ebf5/status`,
    )
...
describe('routing de dominio Supabase', () => {
  it('resuelve el evento de una inscripcion desde Supabase y no desde Prisma', async () => {
    ...
    await expect(
      repository.getRegistrationEventId('11111111-1111-4111-8111-111111111111'),
