...
describe('sesion opaca de atleta', () => {
  it('persiste solo el hash y configura una cookie HttpOnly', async () => {
    let inserted
    const client = { from: () => ({ insert: async (data) => { inserted = data; return { data: null, error: null } } }) }
  ...
  it('rechaza una sesion vencida', async () => {
    const maybeSingle = vi.fn().mockResolvedValue({
      data: { id: 'ses-1', athlete_id: 'ath-1', expires_at: '2026-07-15T00:00:00Z', revoked_at: null },
      error: null,
    })
    const client = {
      from: () => ({ select: () => ({ eq: () => ({ maybeSingle }) }) }),
    }
    const result = await readAthleteSession({ client, token: 'token', now: new Date('2026-07-16T00:00:00Z') })
