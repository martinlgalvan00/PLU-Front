...
const migration = readFileSync(
  resolve(
    process.cwd(),
    'supabase/migrations/20260823110000_registration_commitment_and_public_visibility.sql',
  ),
  'utf8',
)
...
describe('compromiso competitivo y padrón público', () => {
  ...
  it('publica por defecto, permite ocultar desde staff y no altera el cupo', () => {
    ...
    expect(migration).toMatch(
      /and r\.public_visible[\s\S]*and r\.status in \('pendiente_pago', 'pagada', 'confirmada'\)/,
    )
    expect(migration).toMatch(
      /where r\.event_id = v_event\.id\s+and r\.status in \('pendiente_pago', 'pagada', 'confirmada'\);/,
    )
  })
