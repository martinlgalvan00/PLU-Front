...
**Como se modela** (migracion `20260825120000_wise_transfer_channel.sql`,
apoyada sobre el precio configurable por canal de
`20260824100000_manual_price_per_channel.sql` y el settle con cupones de
`20260825100000_promo_codes_fixed_price.sql`):
...
- Interruptor propio `wise_enabled` en Administracion > Acceso y habilitacion,
  **independiente** de `membership_manual_enabled` / `registration_manual_enabled`
  / `ticket_manual_enabled` y del override por cupon que sí aplica a esos dos:
  se puede abrir Wise sin reabrir la transferencia local en ARS.
- Precio fijo en USD por `WISE_PRICE_MEMBERSHIP_USD` / `WISE_PRICE_REGISTRATION_USD`
  ...
  responde `503` en vez de adivinar un monto. No pasa por
  `resolve_channel_price` ni admite cupones: no tiene equivalente ARS.
- `settle_manual_checkout_pricing` resuelve Wise en un branch de salida
  temprana (fija monto + moneda, sin tocar la logica de descuento vigente
  para el resto de los canales). Las tres RPC `_checkout` saltean
  `configure_atomic_checkout_pricing` solo para el canal Wise -- esa funcion
  sigue validando unicamente `bank_transfer`/`cash_pitbull`.
