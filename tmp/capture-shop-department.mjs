...
const fixture = pathToFileURL(path.join(__dirname, 'shop-department-fixture.html')).href
...
for (const vp of viewports) {
  ...
  await page.screenshot({
    path: path.join(__dirname, `shop-department-${vp.name}-dark.png`),
    fullPage: true,
  })
  ...
  await page.screenshot({
    path: path.join(__dirname, `shop-department-${vp.name}-light.png`),
    fullPage: true,
  })

  await page.evaluate(() => {
    document.documentElement.setAttribute('data-theme', 'dark')
    const link = [...document.querySelectorAll('link')].find((el) => el.href.includes('themes/light.css'))
