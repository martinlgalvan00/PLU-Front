...
export const registrationAccessGateSchema = z.object({
  ...
  if (value.scope === 'membership' && value.eventSlug) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['eventSlug'], message: 'La afiliación no lleva torneo.' })
  }
  if (value.startsAt && value.endsAt && new Date(value.endsAt) <= new Date(value.startsAt)) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ['endsAt'], message: 'El cierre debe ser posterior a la apertura.' })
  }
})
...
export function createRegistrationAccessRoutes({ getPrisma, getSupabaseAdmin, repository }) {
  ...
  const repo = () => repository ?? createSupabaseRegistrationAccessRepository(requireSupabaseClient(getSupabaseAdmin()))
  ...
  router.put('/', ...writeGuard, staffLimiter, validateBody(registrationAccessGateSchema), async (req, res, next) => {
    try {
