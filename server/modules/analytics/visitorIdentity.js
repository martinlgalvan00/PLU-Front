...
function saltSecret(env) {
  return (
    env.ANALYTICS_SALT_SECRET?.trim()
    || env.AUTH_SECRET?.trim()
    || 'plu-analytics-dev-salt'
  )
}
...
const BOT_PATTERN = /bot|crawler|spider|crawling|slurp|bingpreview|headless|lighthouse|pingdom|curl|wget|python-requests|axios/i
