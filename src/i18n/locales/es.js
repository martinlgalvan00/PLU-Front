export default {
  brand: {
    name: 'PLU ARG',
    tagline: 'Maximal',
  },
  nav: {
    home: 'Inicio',
    members: 'Afiliación',
    pitbull: 'Pitbull Classic',
    events: 'Eventos',
    results: 'Resultados',
    rulebook: 'Reglamento',
    community: 'Comunidad',
    faq: 'FAQ',
    contact: 'Contacto',
    more: 'Más',
    register: 'Registrarme',
    login: 'Acceder',
    admin: 'Panel',
  },
  hero: {
    eyebrow: 'Federación argentina · Maximal',
    title: 'PLU ARG',
    subtitle: 'Powerlifting con estándar internacional',
    description:
      'Afiliaciones, competencias, resultados y comunidad para atletas de fuerza en Argentina.',
    ctaRegister: 'Registrarme',
    ctaRegisterHint: 'Afiliación anual 2026',
    ctaPitbull: 'Inscribirme a Pitbull Classic',
    ctaPitbullShort: 'Pitbull Classic',
    ctaPitbullHint: 'Meet insignia · agosto 2026',
    ctaPitbullAction: 'Inscribirme',
    quickNavLabel: 'Accesos rápidos',
    ctaLogin: 'Acceder al panel',
    ctaEvents: 'Ver eventos',
    ctaResults: 'Ver resultados',
  },
  theme: {
    label: 'Tema',
    dark: 'Oscuro',
    light: 'Claro',
    system: 'Sistema',
  },
  locale: {
    label: 'Idioma',
    es: 'Español',
    en: 'English',
  },
  common: {
    loading: 'Cargando…',
    retry: 'Reintentar',
    export: 'Exportar',
    save: 'Guardar',
    cancel: 'Cancelar',
    back: 'Volver',
    continue: 'Continuar',
    search: 'Buscar',
    empty: 'Sin resultados',
    errorTitle: 'No pudimos cargar esta sección',
    errorMessage: 'Revisá tu conexión e intentá de nuevo.',
  },
  states: {
    pending_payment: 'Pago pendiente',
    active: 'Activa',
    approved: 'Aprobado',
    confirmed: 'Confirmada',
    rejected: 'Rechazado',
    cancelled: 'Cancelada',
  },
  payment: {
    amount: 'Monto',
    method: 'Método',
    reference: 'Referencia',
    status: 'Estado del pago',
    pendingNote: 'Tu pago está siendo procesado.',
    approvedNote: 'Pago confirmado. Tu trámite quedó activo.',
    rejectedNote: 'El pago no pudo confirmarse. Contactá soporte.',
  },
  confirmation: {
    title: 'Trámite registrado',
    subtitle: 'Recibimos tu solicitud correctamente.',
    nextSteps: 'Próximos pasos',
    viewPanel: 'Ver en panel',
    backHome: 'Volver al inicio',
  },
  admin: {
    dashboard: 'Dashboard',
    searchPlaceholder: 'Buscar atleta, DNI o evento…',
    alerts: 'Alertas operativas',
    exit: 'Volver al sitio',
  },
  footer: {
    ctaEyebrow: 'Temporada 2026',
    ctaTitle: '¿Listo para competir con PLU ARG?',
    ctaDesc: 'Afiliate, inscribite a Pitbull Classic o accedé al panel operativo.',
    brandDesc: 'Federación argentina de powerlifting. Operación junto a Maximal.',
    badgeStandard: 'Estándar internacional',
    navPlatform: 'Plataforma',
    navFederation: 'Federación',
    contact: 'Contacto',
    contactHours: 'Respuesta en 24–48 h hábiles',
    tagline: 'Powerlifting con estándar internacional',
  },
}
