-- Precios temporada 2026: afiliacion automatica (recurring) a $75.000
--
-- Afiliacion one-time: $75.000 (reafirma si el entorno quedo a medias)
-- Afiliacion recurring (plu-annual-auto): $75.000 (nueva version; no muta in-place)
...
declare
  ...
  v_one_time public.membership_plans;
  v_recurring public.membership_plans;
  v_event public.events;
  v_version int;
  v_code text;
  v_recurring_plan_id uuid;
  v_recurring_plan_code text;
  v_effective timestamptz := now();
  v_ends timestamptz := timestamptz '2026-08-28 23:59:59-03';
begin
  -- ---------------------------------------------------------------------------
  -- 1) Plan one-time vigente (reafirmar 75.000 sin mutar filas ligadas a MP)
  -- ---------------------------------------------------------------------------
  ...
  -- ---------------------------------------------------------------------------
  -- 2) Plan recurring plu-annual-auto → 75.000 (versionar; no UPDATE price)
  -- ---------------------------------------------------------------------------
  select p.* into v_source
  from public.membership_plans p
  where p.organization_id = v_org
    and p.collection_mode = 'recurring'
    and p.billing_frequency = 'annual'
    and (
      p.family_code = 'plu-annual-auto'
      or p.code = 'plu-annual-auto'
      or p.family_code like 'plu-annual-auto%'
    )
    and p.active = true
    and (p.retired_at is null or p.retired_at > v_effective)
  order by
    case when p.family_code = 'plu-annual-auto' then 0 else 1 end,
    p.version desc
  limit 1;

  if v_source.id is null then
    raise notice 'No hay plan recurring anual activo; se omite versionado recurring.';
  elsif v_source.price is distinct from 75000 then
    select coalesce(max(version), 0) + 1 into v_version
    from public.membership_plans
    where organization_id = v_org and family_code = v_source.family_code;
    ...
    insert into public.membership_plans (
      ...
      'recurring',
      greatest(coalesce(v_source.interval_count, 1), 1),
      coalesce(v_source.grace_days, 10),
      v_effective,
      true
    )
    ...
    v_recurring_plan_id := v_recurring.id;
    v_recurring_plan_code := v_recurring.code;
  else
    v_recurring_plan_id := v_source.id;
    v_recurring_plan_code := v_source.code;
  end if;

  -- ---------------------------------------------------------------------------
  -- 3) Evento + combo (idempotente)
  -- ---------------------------------------------------------------------------
  select * into v_event
  from public.events
  ...
  insert into public.event_combo_offers (
    ...
    v_org, v_event.id, v_one_time.id, 120000, 'ARS',
  ...
  insert into public.domain_audit_logs (
    ...
    v_org,
    'pricing.recurring_75k_2026_applied',
    'event',
    v_event.id::text,
    'system',
    'migration:20260812160000',
    jsonb_build_object(
      'membershipPrice', 75000,
      'recurringMembershipPrice', 75000,
      'registrationPrice', 75000,
      'comboPrice', 120000,
      'comboEndsAt', v_ends,
      'oneTimePlanId', v_one_time.id,
      'oneTimePlanCode', v_one_time.code,
      'recurringPlanId', v_recurring_plan_id,
      'recurringPlanCode', v_recurring_plan_code
    )
  );
end;
$$;
