-- single_featured_event — PLU ARG
--
-- staff_upsert_event ya guarda `featured` dentro de `rules` (ver
-- 20260716000000_infrastructure_hardening.sql), pero no impedía que dos
-- eventos quedaran con `featured = true` a la vez: el admin podía destacar
-- un evento nuevo sin que el anterior se desmarcara, y el sitio público
-- (getFeaturedEvent = primer `featured` que encuentra) terminaba mostrando
-- cualquiera de los dos según el orden de la lista — inconsistente entre
-- tienda, calendario y home.
--
-- Redefine staff_upsert_event para que, cuando el evento que se guarda
-- queda `featured = true`, se desmarque `featured` en cualquier otro
-- evento que lo tuviera activo, en la misma transacción del upsert.
...
begin
  ...
  -- Un solo evento destacado a la vez: si este upsert lo deja featured,
  -- se lo saca a cualquier otro que lo tuviera.
