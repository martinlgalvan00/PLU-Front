-- Reanudar checkout de inscripción pendiente — PLU ARG
--
-- create_competition_registration_v2 y create_membership_registration_combo_
-- order_core devolvían PLU08 ante cualquier inscripción no cancelada, incluida
-- una `pendiente_pago`. El atleta que reintentaba (o cambiaba Mercado Pago →
-- link de pago) veía "Ya estas inscripto" en vez de las instrucciones de
-- transferencia. create_membership_order_v2 ya reusa la orden abierta; estas
-- dos no.
--
-- Fix: si la inscripción sigue impaga, devolver esa orden. Si el medio nuevo
-- no llegó al proveedor (sin preference, comprobante ni intento embedded),
-- actualizar method/status/expires_at in-place. Si la orden está muerta,
-- cancelar la inscripción residual para que el INSERT posterior pueda
-- recrearla. pagada/confirmada/observada siguen en PLU08.
...
begin
  ...
  v_can_switch :=
    ...
    and v_order.provider_preference_id is null
