...
--
-- Se apoya en dos sistemas que ya existen, sin reemplazarlos:
--   * La matriz concepto x canal (20260826110000_payment_channel_matrix.sql):
--     agrega 'wise_transfer' como cuarto canal, así que hereda gratis
--     interruptor por concepto (afiliación/inscripción/entradas), getter,
--     setter (`staff_set_payment_channel`) y disponibilidad pública.
--   * El precio configurable por canal + settle con cupones
--     (20260824100000_manual_price_per_channel.sql,
--     20260825100000_promo_codes_fixed_price.sql): Wise no tiene equivalente
--     ARS ni admite cupones, así que resuelve su propio precio en USD con un
--     branch de salida temprana en `settle_manual_checkout_pricing`, antes de
--     tocar `resolve_channel_price`/cupones.
--
-- Se modela como un canal más de `manual_link` (lado atleta) / `manual`
-- (lado entradas), igual que 'cash_pitbull': reusa comprobante + aprobación
-- staff sin tocar `approve_athlete_payment_order` (sólo hace excepción
-- explícita para 'cash_pitbull', así que Wise cae del lado "requiere
-- comprobante").
--
-- IMPORTANTE: `CREATE OR REPLACE FUNCTION` con un parámetro nuevo NO
-- reemplaza la función existente -- crea un overload aparte (el equipo ya
-- pisó este problema dos veces: 20260824120000 y 20260824130000). Por eso
-- `settle_manual_checkout_pricing` y las tres RPC "_checkout" llevan su
-- `drop function if exists` con la firma vigente antes del `create or
-- replace` con la firma nueva.

-- ---------------------------------------------------------------------------
-- 1. Matriz concepto x canal: agregar 'wise_transfer'.
-- ---------------------------------------------------------------------------
...
-- Nace cerrado en los tres conceptos: a diferencia de bank_transfer/
-- cash_pitbull (que heredaron el estado del interruptor manual viejo), Wise
-- no existía antes y no hay comportamiento previo que preservar.
...
as $$
  select jsonb_build_object(
    'membership', jsonb_build_object(
      'mercado_pago', true, 'bank_transfer', false, 'cash_pitbull', false, 'wise_transfer', false
    ),
    'registration', jsonb_build_object(
      'mercado_pago', true, 'bank_transfer', false, 'cash_pitbull', false, 'wise_transfer', false
    ),
    'ticket', jsonb_build_object(
      'mercado_pago', true, 'bank_transfer', true, 'cash_pitbull', false, 'wise_transfer', false
    )
  );
$$;

-- 2. Canal manual por orden (athlete_payment_orders): ampliar el CHECK
-- existente (sin cambios desde 20260819130000).
...
-- 3. ticket_orders no tenía noción de canal (provider sólo distingue
-- mercado_pago/manual/mock). Se agrega la columna siguiendo el mismo patrón
-- que el lado atleta, con backfill para las órdenes manuales viejas.
...
begin
  foreach v_concept in array plu_private.payment_channel_concepts() loop
    if jsonb_typeof(v_channels -> v_concept -> 'wise_transfer') <> 'boolean' then
      raise exception 'La celda %/wise_transfer no quedó expuesta como booleano.', v_concept
        using errcode = 'PLU01';
    end if;
    if (v_channels -> v_concept ->> 'wise_transfer')::boolean is distinct from false then
      raise exception 'wise_transfer para % debería nacer cerrado.', v_concept
        using errcode = 'PLU01';
    end if;
  end loop;
