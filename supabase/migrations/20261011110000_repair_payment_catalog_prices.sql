-- Reparacion idempotente para instalaciones donde Vercel se desplego sin
-- aplicar la migracion de Supabase de precios por canal.
...
create or replace function public.staff_set_membership_plan_wise_price(
  p_plan_id uuid, p_wise_price int, p_actor text
)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_plan public.membership_plans;
begin
  ...
  update public.membership_plans set wise_price = p_wise_price, updated_at = now()
...
create or replace function public.staff_set_event_registration_wise_price(
  p_event_slug text, p_wise_price int, p_actor text
)
returns jsonb language plpgsql security definer set search_path = public as $$
...
begin
  ...
  update public.events set wise_price = p_wise_price, updated_at = now()
  where slug = p_event_slug returning * into v_event;
...
grant execute on function public.staff_set_membership_plan_wise_price(uuid, int, text) to service_role;
revoke all on function public.staff_set_event_registration_wise_price(text, int, text)
  from public, anon, authenticated;
grant execute on function public.staff_set_event_registration_wise_price(text, int, text) to service_role;
