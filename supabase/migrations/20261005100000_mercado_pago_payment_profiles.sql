-- Perfiles Mercado Pago por evento (Fase B3) — PLU ARG
--
-- Extiende payment_profiles con ciphertext de secrets (access token + webhook
-- secret) y permite vincular un perfil MP al evento. El public key y collector
-- viven en config (no secretos). La clave de cifrado es PAYMENT_PROFILE_SECRETS_KEY.

alter table public.payment_profiles
  add column if not exists secrets_ciphertext text;

comment on column public.payment_profiles.secrets_ciphertext is
  'AES-256-GCM (PAYMENT_PROFILE_SECRETS_KEY). JSON {accessToken,webhookSecret}. Nunca exponer al browser.';

alter table public.events
  add column if not exists mercado_pago_profile_id uuid
    references public.payment_profiles(id) on delete set null;

create index if not exists events_mercado_pago_profile_id_idx
  on public.events (mercado_pago_profile_id)
  where mercado_pago_profile_id is not null;

comment on column public.events.mercado_pago_profile_id is
  'Perfil MP del evento. NULL = cuenta global del entorno (MERCADO_PAGO_*).';

-- staff_upsert_event: aceptar mercadoPagoProfileId
...
begin
  if not exists (
    ...
    where table_schema = 'public' and table_name = 'payment_profiles'
      and column_name = 'secrets_ciphertext'
  ) then
    raise exception 'Falta payment_profiles.secrets_ciphertext.';
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'events'
      and column_name = 'mercado_pago_profile_id'
  ) then
    raise exception 'Falta events.mercado_pago_profile_id.';
  end if;
end;
$verification$;
