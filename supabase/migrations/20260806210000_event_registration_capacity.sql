-- get_event_registration_capacity — PLU ARG
--
-- Summary público de cupos de atletas + últimos inscriptos sanitizados.
-- Mismo patrón que get_event_ticket_availability: SECURITY DEFINER, solo
-- service_role (Express), sin exponer filas crudas ni PII (DNI/email).
-- displayName = nombre(s) + inicial del apellido; gym opcional.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      order by item.registered_at desc
    ),
    '[]'::jsonb
  )
  into v_recent
  from (
    select
      case
        ...
        else array_to_string(np.parts[1:cardinality(np.parts) - 1], ' ')
          || ' '
          || upper(left(np.parts[cardinality(np.parts)], 1))
          || '.'
      end as display_name,
    ...
    cross join lateral (
      select string_to_array(
        trim(both from regexp_replace(coalesce(a.full_name, ''), '\s+', ' ', 'g')),
        ' '
      ) as parts
    ) np
    where r.event_id = v_event.id
  ...
  return jsonb_build_object(
    ...
    'remaining', case
      ...
      else greatest(v_capacity - v_registered, 0)
    end,
    'recent', v_recent
  );
end;
