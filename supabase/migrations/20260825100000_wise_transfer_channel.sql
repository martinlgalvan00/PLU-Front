...
--
-- Se modela como un canal más de `manual_link` (lado atleta) / `manual`
-- (lado entradas), no como un medio/proveedor nuevo — mismo patrón que
-- `cash_pitbull` (20260819130000). Reusa comprobante + aprobación staff sin
-- tocar `approve_athlete_payment_order`: esa función sólo hace excepción
-- explícita para `cash_pitbull`, así que Wise ya cae del lado "requiere
-- comprobante".
--
-- El precio de Wise es en USD, fijado por la API (no por el cliente) y
-- pasado aparte de la política de preventa ARS (`configure_atomic_checkout_
-- pricing`, 20260819190000), que valida un monto exacto contra una tabla
-- fija en pesos y hoy sólo conoce 'bank_transfer'/'cash_pitbull'. En vez de
-- mezclar dos monedas ahí adentro, las tres RPC "_checkout" saltean esa
-- validación cuando el canal es 'wise_transfer' y fijan precio + moneda
-- únicamente vía `settle_manual_checkout_pricing` (mismo mecanismo que ya
-- usa el cambio de canal transferencia<->efectivo sobre una orden abierta).
...
-- mercado_pago/manual/mock). Se agrega la columna siguiendo el mismo patrón,
-- con backfill para que las órdenes manuales viejas queden consistentes.
...
-- abrir Wise sin reabrir la transferencia local en ARS (cerrada a propósito
-- en 20260823100000). Por eso, a diferencia de los demás boolean de esta
-- tabla, nace en `false` — Wise no existía antes y no hay comportamiento
...
-- `staff_get_platform_feature_toggles` deja todo abierto por defecto cuando
-- no hay fila (20260819180000). Wise es la excepción: nace cerrada incluso
-- sin fila, para no exponer un canal sin datos de cuenta configurados.
...
-- `staff_set_platform_feature_toggle` no cambia: ya es genérica sobre los dos
-- helpers de arriba.

-- 4. `settle_manual_checkout_pricing`: agrega moneda. Parámetro nuevo al
-- final con default para no romper el único caller existente hasta que se
-- actualice más abajo.
...
revoke all on function plu_private.settle_manual_checkout_pricing(uuid, text, text, numeric, text)
  from public, anon, authenticated;

-- 5. Las tres RPC "_checkout": agregan `p_order_currency` (default null,
-- ARS implícito para los caminos existentes) y saltean la política de
-- preventa ARS cuando el canal es Wise.
...
begin
  ...
  v_order := plu_private.settle_manual_checkout_pricing(
    ...
    case when p_manual_payment_channel = 'wise_transfer' then p_order_currency else null end
  );
...
revoke all on function public.create_membership_order_checkout(uuid, text, text, text, text, numeric, text, text)
  from public, anon, authenticated;
grant execute on function public.create_membership_order_checkout(uuid, text, text, text, text, numeric, text, text)
  to service_role;

revoke all on function public.create_competition_registration_checkout(
  uuid, text, text, text, numeric, text, text, text, numeric, text, text
) from public, anon, authenticated;
...
-- 6. Entradas: `create_ticket_order_v2` ya recibe `provider` adentro de
-- `p_buyer` (jsonb) en vez de como parámetro propio — se agrega el canal en
-- el mismo lugar para no cambiar la firma de la función.
