-- Cupo lleno → PLU04 en create_competition_registration_v2 — PLU ARG
--
-- `20260807140000_event_state_control` sincroniza el estado a `agotado` apenas
-- se completa el cupo. El RPC de inscripción seguía tratando cualquier estado
-- fuera de `inscripcion_abierta` / `cupos_limitados` como PLU03 ("no está
-- abierta"), así que el atleta que llegaba justo después del umbral veía
-- "inscripción cerrada" en vez de "no quedan cupos" (PLU04).
--
-- Este parche distingue `agotado` y lo reporta con el mismo código/mensaje
-- que el chequeo de `capacity` más abajo. El conteo de cupo se mantiene como
-- red de seguridad para carreras donde el trigger todavía no movió el estado.
