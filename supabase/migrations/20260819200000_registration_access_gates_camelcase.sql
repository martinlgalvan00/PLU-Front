-- `staff_get_registration_access_gates` devolvía `to_jsonb(g) - 'code_hash'`,
-- es decir las columnas tal cual (snake_case: starts_at, ends_at, created_by,
-- updated_by...). El panel (`RegistrationAccessSection.jsx`) lee camelCase
-- (`gate.startsAt`, `gate.endsAt`), igual que el resto de las respuestas del
-- backend (ver `staff_get_platform_feature_toggles`). Resultado: la fecha de
-- apertura/cierre de una tanda nunca se reflejaba en el panel -- el estado
-- quedaba siempre en "Abierta" aunque estuviera programada o vencida, y al
-- reabrir el formulario de edición las fechas volvían vacías.
...
begin
  ...
  return jsonb_build_object(
    ...
    'scope', v_gate.scope,
    'label', v_gate.label,
    'active', v_gate.active,
    'startsAt', v_gate.starts_at,
    'endsAt', v_gate.ends_at,
    'createdBy', v_gate.created_by,
    'updatedBy', v_gate.updated_by,
    'createdAt', v_gate.created_at,
