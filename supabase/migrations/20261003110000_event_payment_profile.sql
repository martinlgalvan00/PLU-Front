-- Perfil de cobro por evento (Fase A) — PLU ARG
--
-- La matriz `platform_payment_channels` sigue siendo el techo. Por evento se
-- puede restringir canales (apagar MP, dejar sólo transferencia, etc.) y
-- definir alias/CBU/titular propios. Sin multi-cuenta Mercado Pago.

alter table public.events
  add column if not exists payment_channel_overrides jsonb,
  add column if not exists bank_transfer_alias text,
  add column if not exists bank_transfer_cbu text,
  add column if not exists bank_transfer_holder text;

comment on column public.events.payment_channel_overrides is
  'Override opcional de canales para inscription/ticket: {mercado_pago,bank_transfer,cash_pitbull,wise_transfer}. NULL = heredar plataforma. Solo puede cerrar.';
comment on column public.events.bank_transfer_alias is
  'Alias bancario del evento. NULL = usar VITE_PAYMENT_TRANSFER_ALIAS / PAYMENT_TRANSFER_ALIAS.';
comment on column public.events.bank_transfer_cbu is
  'CBU/CVU del evento. NULL = global.';
comment on column public.events.bank_transfer_holder is
  'Titular de la cuenta del evento. NULL = global.';

alter table public.events
  drop constraint if exists events_payment_channel_overrides_object;
alter table public.events
  add constraint events_payment_channel_overrides_object
  check (
    payment_channel_overrides is null
    or jsonb_typeof(payment_channel_overrides) = 'object'
  );

-- ── staff_upsert_event: persistir perfil de cobro ──
-- Redefinición de 20260920100000 + columnas nuevas.
...
begin
  ...
  -- NULL explícito (json null) o clave ausente = heredar plataforma.
  ...
  insert into public.events(
    ...
    bank_transfer_alias = case
      when p_event ? 'bankTransfer' then excluded.bank_transfer_alias
      else public.events.bank_transfer_alias
    end,
    bank_transfer_cbu = case
      when p_event ? 'bankTransfer' then excluded.bank_transfer_cbu
      else public.events.bank_transfer_cbu
    end,
    bank_transfer_holder = case
      when p_event ? 'bankTransfer' then excluded.bank_transfer_holder
...
begin
  if not exists (
    ...
    where table_schema = 'public' and table_name = 'events'
      and column_name = 'payment_channel_overrides'
  ) then
    raise exception 'Falta events.payment_channel_overrides.';
  end if;
  if to_regprocedure('public.staff_upsert_event(jsonb,text)') is null then
    raise exception 'Falta staff_upsert_event.';
  end if;
end;
$verification$;
