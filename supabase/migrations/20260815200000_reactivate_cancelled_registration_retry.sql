-- Reintento de inscripcion cancelada sin violar unique(event_id, athlete_id).
--
-- La tabla conserva una restriccion unica por atleta/evento. La RPC anterior
-- dejaba de considerar `cancelada` como duplicado, pero despues insertaba una
-- fila nueva y chocaba contra event_registrations_event_id_athlete_id_key.
-- En un reintento se debe crear una orden nueva y reactivar la misma fila.
